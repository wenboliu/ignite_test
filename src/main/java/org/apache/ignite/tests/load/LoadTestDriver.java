...
public abstract class LoadTestDriver {
    ...
    /** */
    public void runTest(String testName, Class<? extends Worker> clazz, RuntimeManagerFactory runtimeManagerFactory, String logName, RunningContextFactory runningContextFactory) {
        logger().info("Running " + testName + " test");

        List<RuntimeManager> runtimeManagers = Lists.newArrayList();
        ...
        try {
            ...
            int threadsCount = TestsHelper.getLoadTestsThreadsCount();
            for (int i = 0; i < threadsCount; i++) {
                RuntimeManager runtimeManager = createRuntimeManager(testName, runtimeManagerFactory);
                runtimeManagers.add(runtimeManager);
                Worker worker = createWorker(clazz, runtimeManager, runningContextFactory.createRunningContext(i));
                workers.add(worker);
            ...
            for (RuntimeManager runtimeManager : runtimeManagers) {
                tearDown(runtimeManager);
            }
        }
    }

    private RuntimeManager createRuntimeManager(String testName, RuntimeManagerFactory runtimeManagerFactory) {
        ...
        for (attempt = 0; attempt < NUMBER_OF_SETUP_ATTEMPTS; attempt++) {
            try {
                runtimeManager = setup(runtimeManagerFactory);
                break;
            }
            catch (Throwable e) {
        ...
        if (runtimeManager.isStarted() && attempt == NUMBER_OF_SETUP_ATTEMPTS) {
    ...
    protected abstract RuntimeManager setup(RuntimeManagerFactory runtimeManagerFactory);
    ...
    private Worker createWorker(Class clazz, RuntimeManager runtimeManager, RunningContext runningContext) {
        try {
            Constructor ctor = clazz.getConstructor(RuntimeManager.class, RunningContext.class);

            return (Worker)ctor.newInstance(runtimeManager, runningContext);
        }
        catch (Throwable e) {
